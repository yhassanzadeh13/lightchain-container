...
public class NetworkIntermediary implements LightChainRMIInterface, RMIInterface {
    ...
    public void insertIntoTable(NodeInfo node, int minLevel, NodeInfo insertedNode) throws RemoteException {
        network.insertIntoTable(node, minLevel, insertedNode, node.getNumID());
    }

    @Override
    public boolean unlock(NodeInfo insertedNode, int numID) throws RemoteException {
        return network.unlock(insertedNode, numID, node.getNumID());
    }

    @Override
    public boolean tryAcquire(NodeInfo insertedNode, int numID) throws RemoteException {
        return network.tryAcquire(insertedNode, numID, node.getNumID());
    }
