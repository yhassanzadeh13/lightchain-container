    ...
    public Transaction makeTransaction(String cont) {
        try {
            ...
            while (!success && waitCount > 0) {
                ...
                logger.debug("Prev found: " + (lstBlk != null ? lstBlk.getNumID() : "null"));
...
//			int num = Integer.parseInt(flag.getNameID(), 2);
            int num = Integer.parseInt(flag, 2);
    ...
    public boolean validateTransaction(Transaction t) throws RemoteException {
        ...
        try {
            ...
            logger.debug(String.format("validated: %b, isAuthenticated: %b, isSound: %b, isCorrect: %b, hasBalance: %b",
                    validated, isAuthenticated, isSound, isCorrect, hasBalance));
            return validated;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
