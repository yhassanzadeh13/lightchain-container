...
public class SkipNode extends UnicastRemoteObject implements RMIInterface{
	 ...
	 */
	private static String getHash(String input, int neededSize) { //Returns a binary hash of input with required size
	...
	private static String getHash(String input) { //Returns the SHA-256 hash of the input
		MessageDigest dig; 
		try {
		...
		byte[] encodedhash = dig.digest(input.getBytes(StandardCharsets.UTF_8));
		return bytesToBinary(encodedhash);

	}
	
	private static String bytesToBinary(byte[] hash) {//Converts an array of bytes to the corresponding binary representation.
