...
//							log("Data node with given index does not exist");
						...
						grabUniqueNodes();
						Configuration.generateConfigFile(nodeList, System.getProperty("user.dir") + File.separator
								+ "Logs" + File.separator + "Configurations" + File.separator + "node" + ind + ".conf");
						grabAllNodes();
	...
	private static void printTestingLog() {
		try {
			File logPath = new File(System.getProperty("user.dir") + File.separator + "Logs" + File.separator
					+ "Simulations" + File.separator + "TestingLog" + System.currentTimeMillis() % 200 + ".csv");
			...
			for (NodeInfo cur : TestingLogMap.keySet()) {
				...
				sb.append(
						cur.getNumID() + "," + lg.isMalicious() + "," + lg.getAttempts() + "," + lg.getSuccess() + ",");
	...
	private static boolean printProgress = true;// Whether you want the pingstats function to print % done while called
												// or not.
	...
	public static void pingStats() {
		...
		try {
			File pingLog = new File(System.getProperty("user.dir") + File.separator + "Logs" + File.separator
					+ "RTTDelay" + File.separator + "log" + System.currentTimeMillis() % 20 + ".csv");
	...
	private static void printAllErrors() {
		try {
			grabUniqueNodes();
			File errordir = new File(System.getProperty("user.dir") + File.separator + "Logs" + File.separator
					+ "Errors" + File.separator + "ErrorLog.txt");
			File excepdir = new File(System.getProperty("user.dir") + File.separator + "Logs" + File.separator
					+ "Exceptions" + File.separator + "ExceptionLog.txt");
...
//				}
	...
	private static boolean validateIP(String adrs) {
		...
		String ip = adrs;
		if (colonIndex != -1)
			ip = adrs.substring(0, colonIndex);
